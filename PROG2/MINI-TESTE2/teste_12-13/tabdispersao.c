/*****************************************************************/
/* Trabalho pratico 2 - Tab. Dispersao | PROG2 | MIEEC | 2012/13 */
/*****************************************************************/
...
int tabela_adiciona(tabela_dispersao *td, const objeto *obj)
{
	int index;
	elemento * elem;

	if (!td || !obj) return TABDISPERSAO_ERRO;
	
	/* calcula hash para a string a adicionar */
	index = td->hfunc(obj->chave, td->tamanho);

	/* verifica se chave ja' existe na lista */
	elem = td->elementos[index];
	while (elem != NULL && strcmp(elem->obj->chave, obj->chave)!=0)
		elem = elem->proximo;
	...
	if (elem == NULL) {
		...
		/*copia chave e valor */
		strcpy(elem->obj->chave, obj->chave);
		strcpy(elem->obj->valor, obj->valor);
...
char* tabela_valor(tabela_dispersao *td, const char* chave)
{
	int index;
	elemento * elem;
...
void mostraTabela(tabela_dispersao * td)
{
	...
	for (i = 0; i < td->tamanho; i++)
	{
		if (td->elementos[i])
		{
			...
			{
				valor = elem->obj;
				printf(" : [\"%s\" :: \"%s\"]", valor->chave, valor->valor);
				elem = elem->proximo;
			}
			printf("\n");
		}
