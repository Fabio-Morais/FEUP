/*****************************************************************/
/*         Trabalho pratico 1 | PROG2 | MIEEC | 2012/13          */
/*****************************************************************/
...
vetor* vetor_baralha(vetor* vec)
{
	...
	/* novo vetor onde serao colocados os elementos aleatoriamente */
	novo = vetor_novo();
	if(novo == NULL)
		return NULL;

	/* vetor auxiliar onde serao colocados temporariamente os elementos */
	aux = vetor_novo();
	if(aux == NULL)
		return NULL;

	/* copia vetor original para vetor auxiliar */
	for(i=0; i<vec->tamanho; i++)
	{
		vetor_insere(aux, vetor_elemento(vec,i), -1);
	}

	for(i=0; i<vec->tamanho; i++)
	{
		/* gera posicao aleatoria e verifica se ja foi copiada */
		pos = rand() % aux->tamanho;

		vetor_insere(novo, vetor_elemento(aux, pos), -1);
		vetor_remove(aux, pos);
	}
